...
declare var getonlineregstatus;

@Component({
  selector: 'app-addfeespaymentview',
  templateUrl: './addfeespaymentview.component.html',
  styleUrls: ['./addfeespaymentview.component.scss'],
})
export class AddfeespaymentviewComponent implements OnInit {
 ...
 async loadinit(){
  ...
  geturl(){
    console.log(this.generalservice.apiurl + this.generalservice.viewspecificschoolfeesapi +"?updatetype="+ this.updatetype +"&userid="+ this.userid +"&postid="+ this.postid +"&qstring="+ this.qstring +"&qstringb="+ this.qstringb +"&qstringc="+ this.qstringc +"&qstringd="+ this.qstringd +"&pageNum="+ this.pagenum+"&pageSize="+ this.pagesize+"")
    let defaulturl = this.generalservice.apiurl + this.generalservice.viewspecificschoolfeesapi +"?updatetype="+ this.updatetype +"&userid="+ this.userid +"&postid="+ this.postid +"&qstring="+ this.qstring +"&qstringb="+ this.qstringb +"&qstringc="+ this.qstringc +"&qstringd="+ this.qstringd +"&pageNum="+ this.pagenum+"&pageSize="+ this.pagesize+"";
     return defaulturl; 
  }
...
async loadpost(){
  ...
  .subscribe(async res => {
    ...
    this.Amount= this.featurepost[0].balanceleft;
...
async addfeespayment(){

  if (this.Amount < 1){
    alert("You cannot place a negative amount");
    return
  }

  if(this.Amount > this.featurepost[0].balanceleft){
    alert("The fees you are paying is bigger than the actual fees");
    return
  }
  ...
  var obj = {
    ...
    "specificfeesid":this.featurepost[0].specificfeesid,
    "studentid":this.featurepost[0].studentid,
    "paymentmedium":"1",
    "paymentreference":"2",
    "paymentdescription":"This fees is paid to the cashier",
    "numoftimespaid":"1",
    "paymentreceipturl":"noimage",
  ...
  //Convert the obj to json string
 var jsonData = JSON.stringify(obj);
   this.startloading("Please Wait While we process the fees");
 
  await this.httpClient
     .post(this.generalservice.apiurl + this.generalservice.addfeespayment,jsonData)
     .subscribe(async res => {
        //Connection successful, once you read the data in res you use the status code to perform logic
       console.log(res);
 ...
 openpayment(){
  //alert(this.subnetwork);
  let amount;
  amount =this.featurepost[0].balanceleft *100;
  //alert(this.daysadded)
  payWithPaystack("emmanuelsoligbo@gmail.com",this.featurepost[0].studentFirstname + " " +  this.featurepost[0].studentMiddlename + " " + this.featurepost[0].studentSurname, this.featurepost[0].phone1, amount);
}
