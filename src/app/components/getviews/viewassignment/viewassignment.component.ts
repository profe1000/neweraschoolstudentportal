...
export class ViewassignmentComponent implements OnInit {
 ...
 async loadinit(){
  ...
  geturl(){
    console.log(this.generalservice.apiurl + this.generalservice.viewassignment +"?updatetype="+ this.updatetype +"&userid="+ this.userid +"&postid="+ this.postid  +"&qstring="+ this.qstring +"&qstringb="+ this.qstringb +"&qstringc="+ this.qstringc +"&qstringd="+ this.qstringd +"&pageNum="+ this.pagenum+"&pageSize="+ this.pagesize+"")
    let defaulturl = this.generalservice.apiurl + this.generalservice.viewassignment +"?updatetype="+ this.updatetype +"&userid="+ this.userid +"&postid="+ this.postid  +"&qstring="+ this.qstring +"&qstringb="+ this.qstringb +"&qstringc="+ this.qstringc +"&qstringd="+ this.qstringd +"&pageNum="+ this.pagenum+"&pageSize="+ this.pagesize+"";
     return defaulturl; 
  }
...
if($event.target.value==1){
   ...
   //this.router.navigate(["/quizpageeditor/" + this.featurepost[pageindex].assignmentid +'/' + this.featurepost[pageindex].quizurl])
...
async deletefile(index){
 ...
 //this.startloading();
  ...
  await this.httpClient
     .get(this.generalservice.apiurl + ""+ "?userid=1&assignmentid="+ this.featurepost[index].assignmentid +"")
     .subscribe(async res => {
    ...
    if (res["statuscode"]!= 100){
       this.presentToast("File not deleted");
     }
...
async confirmfile(index){
  ...
  // the post obj
  var obj = {
    "assignmentid":this.featurepost[index].assignmentid,
    "orderstatuscode":1,
    "itemnum":1,
    "apikey":"",
          };
...
loadmyeditfunction(pageindex){
  this.loadeditfunction.emit(this.featurepost[pageindex].assignmentid);
  //alert(this.featurepost[pageindex].assignmentid)
}

loadmyorderdispatch(pageindex){
  this.loadorderdespatch.emit(this.featurepost[pageindex].assignmentid);
}

loadmyordermessage(pageindex){
  this.loadordermessage.emit(this.featurepost[pageindex].assignmentid);
}

  //Used to Start Quiz
  async startassignment(index){
    // the post obj
    var obj = {
      "status":1,
      "assignmentid":this.featurepost[index]["assignmentid"],
   ...
   this.startloading("Please Wait");
    ...
    await this.httpClient
       .post(this.generalservice.apiurl + "",jsonData)
       .subscribe(async res => {
          //Connection successful, once you read the data in res you use the status code to perform logic
         console.log(res);
     ...
     if (res["statuscode"]== 99){
       //this.storage.set('logindetails', res);
        this.presentToast(res["status"]);
        this.featurepost[index]["publishedstatus"] = true;
      }
       ...
       this.startloading("Please Wait");
        ...
        await this.httpClient
         ...
         if (res["statuscode"]== 99){
           //this.storage.set('logindetails', res);
            this.presentToast(res["status"]);
            this.featurepost[index]["publishedstatus"] = false;
          }
