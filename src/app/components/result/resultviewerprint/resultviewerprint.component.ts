...
@Component({
  ...
  templateUrl: './resultviewerprint.component.html',
 ...
 calculatetotalscore(){
  ...
  for(var i =0; i < this.resultdata.post.length; i++){
    if(this.resultdata.post[i].deletestatus == "false" || this.resultdata.post[i].deletestatus == false){
      ...
      if ((this.resultdata.post[i].ca + this.resultdata.post[i].ca2 + this.resultdata.post[i].ca3 + this.resultdata.post[i].exam) >= this.distictionleast){
           this.numberofdistintion =  this.numberofdistintion + 1
      } else   if ((this.resultdata.post[i].ca + this.resultdata.post[i].ca2 + this.resultdata.post[i].ca3 + this.resultdata.post[i].exam) >= this.passleast){
        this.numberofpass =  this.numberofpass + 1
      } else   if ((this.resultdata.post[i].ca + this.resultdata.post[i].ca2 + this.resultdata.post[i].ca3 + this.resultdata.post[i].exam) >= this.failleast){
...
//this.startloading();
 ...
 await this.httpClient
    .post(this.generalservice.apiurl +'api/users/update/updatecart.ashx',jsonData)
    .subscribe(async res => {
       //Connection successful, once you read the data in res you use the status code to perform logic
      console.log(res);
...
return;
  // the post obj
  var obj = {
    "documentbody": document.getElementById("print-section").innerHTML
  };
 ...
 //this.startloading();
  ...
  await this.httpClient
     .post(this.generalservice.apiurl + this.generalservice.generatepdf,jsonData)
     .subscribe(async res => {
        //Connection successful, once you read the data in res you use the status code to perform logic
       console.log(res);
